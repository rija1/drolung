...
class Countries extends \IAWP\Rows\Rows
{
    ...
    private function query(?bool $skip_pagination = \false) : Builder
    {
        ...
        $session_duration_query = Illuminate_Builder::new()->select('sessions.country_id')->selectRaw('AVG(TIMESTAMPDIFF(SECOND, sessions.created_at, sessions.ended_at)) AS average_session_duration')->from(Tables::sessions(), 'sessions')->whereIn('sessions.session_id', function ($subquery) {
        ...
        $countries_query = Illuminate_Builder::new()->select('countries.country_id', 'countries.country_code', 'countries.country', 'countries.continent')->selectRaw('COUNT(DISTINCT views.id) AS views')->selectRaw('COUNT(DISTINCT sessions.visitor_id) AS visitors')->selectRaw('COUNT(DISTINCT sessions.session_id) AS sessions')->selectRaw('MAX(session_durations.average_session_duration) AS average_session_duration')->selectRaw('COUNT(DISTINCT IF(sessions.final_view_id IS NULL, sessions.session_id, NULL)) AS bounces')->from(Tables::sessions() . ' AS sessions')->leftJoin(Tables::views() . ' AS views', 'sessions.session_id', '=', 'views.session_id')->leftJoin(Tables::countries() . ' AS countries', 'sessions.country_id', '=', 'countries.country_id')->leftJoinSub($session_duration_query, 'session_durations', 'sessions.country_id', '=', 'session_durations.country_id')->tap(Query_Taps::tap_authored_content_check())->tap(Query_Taps::tap_related_to_examined_record($this->examiner_config))->whereNotNull('sessions.country_id')->groupBy('sessions.country_id')->having('views', '>', 0)->tap(fn(Builder $query) => $this->apply_record_filters($query))->whereBetween('views.viewed_at', $this->get_current_period_iso_range())->when(!$this->appears_to_be_for_real_time_analytics(), function (Builder $query) {
            $query->whereBetween('sessions.created_at', $this->get_current_period_iso_range());
        })->when(\is_int($this->solo_record_id), function (Builder $query) {
            $query->where('countries.country_id', '=', $this->solo_record_id);
        ...
        })->fromSub($countries_query, 'countries')->leftJoinSub($clicks_subquery, 'click_stats', 'click_stats.country_id', '=', 'countries.country_id')->leftJoinSub($orders_subquery, 'order_stats', 'order_stats.country_id', '=', 'countries.country_id')->leftJoinSub($form_submissions_subquery, 'form_submission_stats', 'form_submission_stats.country_id', '=', 'countries.country_id')->leftJoinSub($previous_period_query, 'previous_period_stats', 'previous_period_stats.country_id', '=', 'countries.country_id')->tap(fn(Builder $query) => $this->apply_aggregate_filters($query))->when(!$this->can_order_and_limit_at_record_level() && !($this->using_logical_or_operator() && $this->filtering_by_mixed_columns()), function (Builder $query) {
            $query->tap(fn(Builder $query) => $this->apply_order_and_limit($query, $this->sort_configuration->column()));
        });
